...
input.addEventListener("input", () => {
  ...
  // ciudades únicas y limpias
  const allCities = [...new Set(centers.map(c => c.city).filter(Boolean))];

  // filtra sin tildes y por prefijo
  const matches = allCities.filter(c => norm(c).startsWith(val)).slice(0, 12);
