...
originSelect.innerHTML = '<option value="">Argentina</option>' +
...
// Auto-redirect if there is exactly one country
if (FIXED_COUNTRIES.length === 1 && !currentOrigin) {
  const q = new URLSearchParams();
  q.set('origin', FIXED_COUNTRIES[0]);
  const existingCity = new URLSearchParams(window.location.search).get('city');
  if (existingCity) q.set('city', existingCity);
